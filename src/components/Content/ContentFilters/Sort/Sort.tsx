...
export const Sort: FC = () => {
    ...
    let sortId = useSelector((state: RootState) => state.filter.sortId);
    const dispatch = useDispatch();

    const sortRef = useRef<HTMLDivElement | null>(null);

    const sortListHanler = (index: number): void => {
        setIsVisible(false);
        dispatch(setSortId(index));
    };

    useEffect(() => {
        // Скрыть выпадающее окно при клике на body
        if (isVisible) {
            const sortClickHandler = (event: MouseEvent): void => {
                if (
                    sortRef.current &&
